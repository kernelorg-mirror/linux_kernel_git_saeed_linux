...
hws_definer_flex_parser_tag_set(struct mlx5hws_definer_conv_data *cd, u8 parser_id)
{
	struct mlx5hws_definer_fc *fc;
		      ...
		      u32 *match_param)
{
	...
	if (HWS_GET_MATCH_PARAM(match_param, misc_parameters.geneve_tlv_option_0_exist)) {
		...
		curr_fc = hws_definer_flex_parser_tag_set(cd,
			     ...
			     u32 *data)
{
	...
	for (i = 0; i < DW_SIZE; i++)
		if ((0xff000000 >> (i * BITS_IN_BYTE)) & be32_to_cpu(*data)) {
	...
	for (i = 0; i < DW_SIZE; i++)
		if ((0xff << (i * BITS_IN_BYTE)) & be32_to_cpu(*data)) {
			ctrl->used_bytes--;
			byte_idx = ctrl->allowed_bytes - ctrl->used_bytes - 1;
